...
def default_logs_dir() -> Path:
    if sys.platform == "win32":
        base = Path(os.environ.get("LOCALAPPDATA", Path.home())) / "LoLAutoDirector" / "logs"
    else:
        base = Path.home() / ".lol-auto-director" / "logs"
...
class GameSessionLogger:
    ...
    def _start_session_unlocked(self, game_time: float) -> None:
        ...
        header = (
            f"=== LoL Auto Director — game session {self._session_start.isoformat(timespec='seconds')} ==="
        )
